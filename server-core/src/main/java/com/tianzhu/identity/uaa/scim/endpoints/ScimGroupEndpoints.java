...
import com.tianzhu.identity.uaa.scim.exception.*;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import com.tianzhu.identity.uaa.resources.AttributeNameMapper;
import com.tianzhu.identity.uaa.resources.SearchResults;
import com.tianzhu.identity.uaa.resources.SearchResultsFactory;
import com.tianzhu.identity.uaa.resources.SimpleAttributeNameMapper;
import com.tianzhu.identity.uaa.resources.jdbc.SimpleSearchQueryConverter;
...
import com.tianzhu.identity.uaa.util.UaaPagingUtils;
...
import static com.tianzhu.identity.uaa.zone.ZoneManagementScopes.ZONE_MANAGING_SCOPE_REGEX;
...
public class ScimGroupEndpoints {
    ...
    @Autowired
    @Qualifier("externalGroupMembershipManager")
    ...
    @Autowired
    public ScimGroupEndpoints(@Qualifier("scimGroupProvisioning") ScimGroupProvisioning scimGroupProvisioning, @Qualifier("groupMembershipManager") ScimGroupMembershipManager membershipManager) {
        this.dao = scimGroupProvisioning;
        this.membershipManager = membershipManager;
        statuses.put(DataIntegrityViolationException.class,BAD_REQUEST);
        statuses.put(HttpMessageConversionException.class,BAD_REQUEST);
        statuses.put(HttpMediaTypeException.class,BAD_REQUEST);
        statuses.put(IllegalArgumentException.class,BAD_REQUEST);
        statuses.put(UnsupportedOperationException.class,BAD_REQUEST);
        statuses.put(InvalidPasswordException.class,BAD_REQUEST);
        statuses.put(InvalidScimResourceException.class,BAD_REQUEST);
        statuses.put(ScimResourceNotFoundException.class,NOT_FOUND);
        statuses.put(ScimResourceAlreadyExistsException.class,CONFLICT);
        statuses.put(ScimResourceConflictException.class,CONFLICT);
        statuses.put(BadSqlGrammarException.class,BAD_REQUEST);
    }
    ...
    public SearchResults<?> listGroups(
        ...
        try {
            ...
            throw new ScimException("Invalid filter expression: [" + filter + "]", BAD_REQUEST);
        }
        ...
        try {
            ...
            throw new ScimException("Invalid attributes: [" + attributesCommaSeparated + "]", BAD_REQUEST);
        }
    }
    ...
    public SearchResults<?> getExternalGroups(
        ...
        if (hasText(filter)) {
            if (hasText(origin) || hasText(externalGroup)) {
                throw new ScimException("Deprecated filter parameter may not be used in conjunction with origin or externalGroup parameters", BAD_REQUEST);
            }
            ...
            try {
                ...
                throw new ScimException("Filter not supported, please use origin and externalGroup parameters", e, BAD_REQUEST);
            }
        }
        ...
        try {
            ...
            throw new ScimException("Invalid filter expression: [" + filter + "]", e, BAD_REQUEST);
        }
    ...
    public ScimGroupExternalMember mapExternalGroup(@RequestBody ScimGroupExternalMember sgm) {
        try {
            ...
            throw new ScimException(e.getMessage(), BAD_REQUEST);
        } catch (ScimResourceNotFoundException e) {
            throw new ScimException(e.getMessage(), NOT_FOUND);
        } catch (MemberAlreadyExistsException e) {
            throw new ScimException(e.getMessage(), CONFLICT);
        }
    }
    ...
    private String getGroupId(String displayName) {
        if (displayName==null || displayName.trim().length()==0) {
            throw new ScimException("Group not found, not name provided", NOT_FOUND);
        }
        List<ScimGroup> result = dao.query("displayName eq \""+displayName+"\"", IdentityZoneHolder.get().getId());
        if (result==null || result.size()==0) {
            throw new ScimException("Group not found:"+displayName, NOT_FOUND);
        }
    ...
    public ScimGroup updateGroup(@RequestBody ScimGroup group, @PathVariable String groupId,
        ...
        if (etag == null) {
            throw new ScimException("Missing If-Match for PUT", BAD_REQUEST);
        }
    ...
    public ScimGroup patchGroup(@RequestBody ScimGroup patch, @PathVariable
                                ...
                                HttpServletResponse httpServletResponse) {
        if (etag == null) {
            throw new ScimException("Missing If-Match for PATCH", BAD_REQUEST);
        }
    ...
    public ScimGroup deleteGroup(@PathVariable String groupId,
        ...
        try {
            ...
            throw new ScimException("error deleting group: " + groupId, ex, CONFLICT);
        }
        return group;
    }
    ...
    public ScimGroup addZoneManagers(@RequestBody ScimGroup group, HttpServletResponse httpServletResponse) {
        if (!group.getDisplayName().matches(ZONE_MANAGING_SCOPE_REGEX)) {
            throw new ScimException("Invalid group name.", BAD_REQUEST);
        }
        if (group.getMembers()==null || group.getMembers().size()==0) {
            throw new ScimException("Invalid group members, you have to add at least one member.", BAD_REQUEST);
        }
        try {
            ...
            if (ex.getStatus().equals(NOT_FOUND)) {
    ...
    public ScimGroup deleteZoneScope(@PathVariable String userId,
        ...
        if (!hasText(userId) || !hasText(zoneId)) {
            throw new ScimException("User ID and Zone ID are required.", BAD_REQUEST);
        }
        if (!isMember(group, userId)) {
            throw new ScimException("User is not a zone admin.", NOT_FOUND);
        }
