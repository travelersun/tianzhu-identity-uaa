...
public class ResetPasswordAuthenticationEntryPointTests {
    ...
    public void setup() throws Exception {
        ...
        when(request.getRequestDispatcher(ArgumentMatchers.anyString())).thenReturn(requestDispatcher);
    ...
    public void test_invalid_password_match() throws Exception {
        ...
        verify(request, times(1)).getRequestDispatcher(ArgumentMatchers.eq("/reset_password"));
        verify(request, times(1)).setAttribute(ArgumentMatchers.eq("message_code"), ArgumentMatchers.eq(messageCode));

        verify(requestDispatcher, timeout(1)).forward(ArgumentMatchers.any(HttpServletRequest.class), ArgumentMatchers.same(response));
        verify(response, times(1)).setStatus(ArgumentMatchers.eq(HttpStatus.UNPROCESSABLE_ENTITY.value()));
    }
    ...
    public void test_when_uaa_exception() throws Exception {
        ...
        verify(request, times(1)).getRequestDispatcher(ArgumentMatchers.eq("/forgot_password"));
        verify(request, times(1)).setAttribute(ArgumentMatchers.eq("message_code"), ArgumentMatchers.eq("bad_code"));
        verify(requestDispatcher, timeout(1)).forward(ArgumentMatchers.any(HttpServletRequest.class), ArgumentMatchers.same(response));
        verify(response, times(1)).setStatus(ArgumentMatchers.eq(HttpStatus.UNPROCESSABLE_ENTITY.value()));
    }
    ...
    public void test_when_invalid_password_exception() throws Exception {
        ...
        verify(request, times(1)).getRequestDispatcher(ArgumentMatchers.eq("/reset_password"));
        verify(request, times(1)).setAttribute(ArgumentMatchers.eq("message"), ArgumentMatchers.eq(pe.getMessagesAsOneString()));
