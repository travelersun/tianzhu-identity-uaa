 ...
 *******************************************************************************/
package com.tianzhu.identity.uaa.authentication.login;

import com.tianzhu.identity.uaa.authentication.AccountNotVerifiedException;
import com.tianzhu.identity.uaa.authentication.RemoteAuthenticationEndpoint;
...
public class RemoteAuthenticationEndpointTests {
    ...
    public void successfulAuthenticationGives200Status() throws Exception {
        ...
        when(am.authenticate(ArgumentMatchers.any(Authentication.class))).thenReturn(success);
    ...
    public void accountNotVerifiedExceptionGives403Status() throws Exception {
        when(am.authenticate(ArgumentMatchers.any(Authentication.class))).thenThrow(new AccountNotVerifiedException("failed"));
        @SuppressWarnings("rawtypes")
        ResponseEntity response = (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe","joespassword");
    ...
    public void authenticationExceptionGives401Status() throws Exception {
        when(am.authenticate(ArgumentMatchers.any(Authentication.class))).thenThrow(new BadCredentialsException("failed"));
        @SuppressWarnings("rawtypes")
        ResponseEntity response = (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe","joespassword");
    ...
    public void otherExceptionGives500Status() throws Exception {
        when(am.authenticate(ArgumentMatchers.any(Authentication.class))).thenThrow(new RuntimeException("error"));
        @SuppressWarnings("rawtypes")
        ResponseEntity response = (ResponseEntity) endpoint.authenticate(new MockHttpServletRequest(), "joe","joespassword");
    ...
    public void successfulLoginAuthenticationInvokesLoginAuthManager() throws Exception {
        ...
        when(am.authenticate(ArgumentMatchers.any(Authentication.class))).thenThrow(new BadCredentialsException("Invalid authentication manager invoked"));
        when(loginAuthMgr.authenticate(ArgumentMatchers.any(Authentication.class))).thenReturn(new UsernamePasswordAuthenticationToken("joe", null));
