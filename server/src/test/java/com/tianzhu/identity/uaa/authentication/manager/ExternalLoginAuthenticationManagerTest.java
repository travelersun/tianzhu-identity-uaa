...
import com.tianzhu.identity.uaa.provider.ExternalIdentityProviderDefinition;
...
import com.tianzhu.identity.uaa.provider.ldap.extension.ExtendedLdapUserImpl;
...
public class ExternalLoginAuthenticationManagerTest  {
    ...
    private UaaUser addUserToDb(String userName, String userId, String origin, String email) {
        ...
        when(this.uaaUserDatabase.retrieveUserById(ArgumentMatchers.eq(userId))).thenReturn(user);
        when(this.uaaUserDatabase.retrieveUserByName(ArgumentMatchers.eq(userName), ArgumentMatchers.eq(origin))).thenReturn(user);
        return user;
    }
    ...
    public void testNoUsernameNoEmail() throws Exception {
        ...
        when(uaaAuthenticationDetails.getSessionId()).thenReturn(new RandomValueStringGenerator().generate());
        when(inputAuth.getDetails()).thenReturn(uaaAuthenticationDetails);
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.anyString(), ArgumentMatchers.eq(origin))).thenReturn(null);
        when(userDetails.getUsername()).thenReturn(null);
    ...
    public void testAmpersandInName() throws Exception {
        ...
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.eq(name), ArgumentMatchers.eq(origin)))
            .thenReturn(null)
            .thenReturn(user);
    ...
    public void testAuthenticateUserInsertFails() throws Exception {
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenThrow(new UsernameNotFoundException(""));
        manager.authenticate(inputAuth);
    }
    ...
    public void testAuthenticateLdapUserDetailsPrincipal() throws Exception {
        ...
        manager.setOrigin(origin);
        when(user.getOrigin()).thenReturn(origin);
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.eq(userName), ArgumentMatchers.eq(origin))).thenReturn(user);
    ...
    public void testShadowUserCreationDisabled() throws Exception {
        ...
        setupManager();
        manager.setOrigin(origin);
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.eq(userName), ArgumentMatchers.eq(origin))).thenReturn(null);
        when(inputAuth.getPrincipal()).thenReturn(ldapUserDetails);
        ...
        verify(applicationEventPublisher, times(0)).publishEvent(ArgumentMatchers.any());
    }
    ...
    public void testAuthenticateCreateUserWithLdapUserDetailsPrincipal() throws Exception {
        ...
        when(user.getEmail()).thenReturn(email);
        when(user.getOrigin()).thenReturn(origin);
        when(uaaUserDatabase.retrieveUserByName(ArgumentMatchers.eq(userName), ArgumentMatchers.eq(origin)))
            .thenReturn(null)
            .thenReturn(user);
        when(inputAuth.getPrincipal()).thenReturn(ldapUserDetails);
    ...
    public void testAuthenticateInvitedUserWithoutAcceptance() throws Exception {
        ...
        when(invitedUser.modifyAttributes(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(invitedUser);
        ...
        when(uaaUserDatabase.retrieveUserByEmail(ArgumentMatchers.eq(email), ArgumentMatchers.eq(origin)))
    ...
    public void testPopulateAttributesStoresCustomAttributesAndRoles() {
        ...
        when(providerProvisioning.retrieveByOrigin(ArgumentMatchers.eq(origin), ArgumentMatchers.anyString())).thenReturn(provider);
        ...
        verify(manager.getUserDatabase(), never()).storeUserInfo(ArgumentMatchers.anyString(), ArgumentMatchers.anyObject());
        ...
        verify(manager.getUserDatabase(), times(1)).storeUserInfo(ArgumentMatchers.eq("id"), ArgumentMatchers.eq(userInfo));
