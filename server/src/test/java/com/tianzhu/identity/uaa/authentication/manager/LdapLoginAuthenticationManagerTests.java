...
import com.tianzhu.identity.uaa.provider.ldap.extension.LdapAuthority;
...
public class LdapLoginAuthenticationManagerTests {
    ...
    public void setUp() {
        ...
        when(db.retrieveUserById(ArgumentMatchers.anyString())).thenReturn(dbUser);
        am.setUserDatabase(db);
        when(auth.getAuthorities()).thenReturn(null);

        provider = mock(IdentityProvider.class);

        when(provisioning.retrieveByOrigin(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(provider);
    ...
    public void test_authentication_attributes(boolean storeUserInfo) throws Exception {
        ...
        UaaUserDatabase db = mock(UaaUserDatabase.class);
        when(db.retrieveUserByName(ArgumentMatchers.anyString(), ArgumentMatchers.eq(OriginKeys.LDAP))).thenReturn(user);
        when(db.retrieveUserById(ArgumentMatchers.anyString())).thenReturn(user);
        ...
        if (storeUserInfo) {
            verify(db, times(1)).storeUserInfo(ArgumentMatchers.anyString(), ArgumentMatchers.eq(info));
        } else {
            verify(db, never()).storeUserInfo(ArgumentMatchers.anyString(), ArgumentMatchers.eq(info));
        }
