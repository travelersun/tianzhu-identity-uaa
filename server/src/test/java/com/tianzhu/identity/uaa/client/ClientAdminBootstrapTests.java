...
import com.tianzhu.identity.uaa.authentication.SystemAuthentication;
...
public class ClientAdminBootstrapTests extends JdbcTestBase {
    ...
    public void client_slated_for_deletion_does_not_get_inserted() throws Exception {
        ...
        verify(clientRegistrationService, never()).addClientDetails(ArgumentMatchers.any(), anyString());
        verify(clientRegistrationService, never()).updateClientDetails(ArgumentMatchers.any(), anyString());
        verify(clientRegistrationService, never()).updateClientSecret(ArgumentMatchers.any(), ArgumentMatchers.any(), anyString());
    }
    ...
    public void test_delete_from_yaml_existing_client() throws Exception {
        ...
        verify(bootstrap, never()).publish(ArgumentMatchers.any());
        ...
        verify(publisher, times(1)).publishEvent(ArgumentMatchers.same(captor.getValue()));
    ...
    public void testSimpleAddClientWithAutoApprove() throws Exception {
        ...
        when(clientMetadataProvisioning.update(ArgumentMatchers.any(ClientMetadata.class), anyString())).thenReturn(new ClientMetadata());
        doReturn(output).when(clientRegistrationService).loadClientByClientId(ArgumentMatchers.eq("foo"), anyString());
        bootstrap.setClients(Collections.singletonMap((String) map.get("id"), map));
    ...
    public void testOverrideClient() throws Exception {
        ...
        doThrow(new ClientAlreadyExistsException("Planned"))
            .when(clientRegistrationService).addClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
        bootstrap.afterPropertiesSet();
        verify(clientRegistrationService, times(1)).addClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
    ...
    public void testOverrideClientByDefault() throws Exception {
        ...
        doThrow(new ClientAlreadyExistsException("Planned")).when(clientRegistrationService)
            .addClientDetails(
                ArgumentMatchers.any(ClientDetails.class),
                anyString()
            );
        bootstrap.afterPropertiesSet();
        verify(clientRegistrationService, times(1)).addClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
        verify(clientRegistrationService, times(1)).updateClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
        verify(clientRegistrationService, times(1)).updateClientSecret("foo", "bar", IdentityZoneHolder.get().getId());
    }
    ...
    public void testOverrideClientWithYaml() throws Exception {
        ...
        doThrow(new ClientAlreadyExistsException("Planned")).when(clientRegistrationService).addClientDetails(
                        ArgumentMatchers.any(ClientDetails.class), anyString());
        bootstrap.afterPropertiesSet();
        verify(clientRegistrationService, times(2)).addClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
        verify(clientRegistrationService, times(2)).updateClientDetails(ArgumentMatchers.any(ClientDetails.class), anyString());
        verify(clientRegistrationService, times(1)).updateClientSecret("foo", "bar", IdentityZoneHolder.get().getId());
