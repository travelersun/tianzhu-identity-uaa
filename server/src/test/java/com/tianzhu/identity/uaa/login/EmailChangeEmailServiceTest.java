...
import com.tianzhu.identity.uaa.account.EmailChangeEmailService;
...
import static com.tianzhu.identity.uaa.account.EmailChangeEmailService.CHANGE_EMAIL_REDIRECT_URL;
...
public class EmailChangeEmailServiceTest {
    ...
    public void beginEmailChange() throws Exception {
        ...
        Mockito.verify(messageService).sendMessage(
                ArgumentMatchers.eq("new@example.com"),
                ArgumentMatchers.eq(MessageType.CHANGE_EMAIL),
                ArgumentMatchers.eq("Account Email change verification"),
                ArgumentMatchers.contains("<a href=\"http://localhost/login/verify_email?code=the_secret_code\">Verify your email</a>")
        );
    }
    ...
    public void beginEmailChangeWithUsernameConflict() throws Exception {
        ...
        when(scimUserProvisioning.retrieve(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(user);
        String zoneId = IdentityZoneHolder.get().getId();
        when(scimUserProvisioning.query(ArgumentMatchers.anyString(), ArgumentMatchers.eq(zoneId))).thenReturn(Collections.singletonList(new ScimUser()));
    ...
    public void testBeginEmailChangeWithCompanyNameConfigured() throws Exception {
        ...
        try {
            ...
            Mockito.verify(messageService).sendMessage(
              ArgumentMatchers.eq("new@example.com"),
              ArgumentMatchers.eq(MessageType.CHANGE_EMAIL),
              ArgumentMatchers.eq("Best Company Email change verification"),
    ...
    public void testBeginEmailChangeInOtherZone(String zoneName) throws Exception {
        ...
        Mockito.verify(messageService).sendMessage(
                ArgumentMatchers.eq("new@example.com"),
                ArgumentMatchers.eq(MessageType.CHANGE_EMAIL),
                ArgumentMatchers.eq(zoneName+" Email change verification"),
                emailBodyArgument.capture()
        );
    ...
    private void setUpForBeginEmailChange() {
        ...
        when(scimUserProvisioning.retrieve("user-001", IdentityZoneHolder.get().getId())).thenReturn(user);
        when(scimUserProvisioning.query(ArgumentMatchers.anyString(), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()))).thenReturn(Collections.singletonList(new ScimUser()));
        String data = JsonUtils.writeValueAsString(codeData);
        when(codeStore.generateCode(ArgumentMatchers.eq(data), ArgumentMatchers.any(Timestamp.class), ArgumentMatchers.eq(EMAIL.name()), ArgumentMatchers.anyString())).thenReturn(new ExpiringCode("the_secret_code", new Timestamp(System.currentTimeMillis()), data, EMAIL.name()));

        emailChangeEmailService.beginEmailChange("user-001", "user@example.com", "new@example.com", "app", "http://app.com");

        verify(codeStore).generateCode(ArgumentMatchers.eq(JsonUtils.writeValueAsString(codeData)), ArgumentMatchers.any(Timestamp.class), ArgumentMatchers.eq(EMAIL.name()), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()));
    }
