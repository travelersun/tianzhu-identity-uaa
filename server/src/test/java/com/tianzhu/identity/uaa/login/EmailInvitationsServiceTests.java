...
import com.tianzhu.identity.uaa.invitations.EmailInvitationsService;
...
import static com.tianzhu.identity.uaa.invitations.EmailInvitationsService.EMAIL;
import static com.tianzhu.identity.uaa.invitations.EmailInvitationsService.USER_ID;
...
public class EmailInvitationsServiceTests {
    ...
    public void acceptInvitationNoClientId() throws Exception {
        ...
        when(scimUserProvisioning.retrieve(ArgumentMatchers.eq("user-id-001"), ArgumentMatchers.eq(zoneId))).thenReturn(user);
        ...
        when(scimUserProvisioning.update(ArgumentMatchers.anyString(), ArgumentMatchers.anyObject(), ArgumentMatchers.eq(zoneId))).thenReturn(user);

        Map<String,String> userData = new HashMap<>();
        userData.put(USER_ID, "user-id-001");
        userData.put(EMAIL, "user@example.com");
        when(expiringCodeStore.retrieveCode(ArgumentMatchers.anyString(), ArgumentMatchers.eq(zoneId))).thenReturn(new ExpiringCode("code", new Timestamp(System.currentTimeMillis()), JsonUtils.writeValueAsString(userData), INVITATION.name()));
    ...
    public void nonMatchingCodeIntent() {
        ...
        userData.put(USER_ID, "user-id-001");
        userData.put(EMAIL, "user@example.com");
        when(expiringCodeStore.retrieveCode(ArgumentMatchers.anyString(), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()))).thenReturn(new ExpiringCode("code", new Timestamp(System.currentTimeMillis()), JsonUtils.writeValueAsString(userData), "wrong-intent"));
    ...
    public void acceptInvitation_withoutPasswordUpdate() throws Exception {
        ...
        verify(scimUserProvisioning).verifyUser(user.getId(), user.getVersion(), zoneId);
        verify(scimUserProvisioning, never()).changePassword(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.eq(zoneId));
    }
    ...
    public void accept_invitation_with_external_user_that_does_not_have_email_as_their_username() {
        ...
        String zoneId = IdentityZoneHolder.get().getId();
        when(scimUserProvisioning.verifyUser(ArgumentMatchers.eq(userId), ArgumentMatchers.anyInt(), ArgumentMatchers.eq(zoneId))).thenReturn(userBeforeAccept);
        when(scimUserProvisioning.retrieve(ArgumentMatchers.eq(userId), ArgumentMatchers.eq(zoneId))).thenReturn(userBeforeAccept);
        ...
        when(scimUserProvisioning.verifyUser(ArgumentMatchers.eq(userId), ArgumentMatchers.anyInt(), ArgumentMatchers.eq(zoneId))).thenReturn(userAfterAccept);
        ...
        verify(scimUserProvisioning).verifyUser(ArgumentMatchers.eq(userId), ArgumentMatchers.anyInt(), ArgumentMatchers.eq(zoneId));
