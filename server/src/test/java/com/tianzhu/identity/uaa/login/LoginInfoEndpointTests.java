...
import com.tianzhu.identity.uaa.cache.UrlContentCache;
...
import com.tianzhu.identity.uaa.provider.oauth.XOAuthProviderConfigurator;
import com.tianzhu.identity.uaa.provider.saml.LoginSamlAuthenticationToken;
import com.tianzhu.identity.uaa.provider.saml.SamlIdentityProviderConfigurator;
...
import static com.tianzhu.identity.uaa.login.LoginInfoEndpoint.OAUTH_LINKS;
import static com.tianzhu.identity.uaa.login.LoginInfoEndpoint.SHOW_LOGIN_LINKS;
import static com.tianzhu.identity.uaa.util.UaaUrlUtils.addSubdomainToUrl;
...
public class LoginInfoEndpointTests {
    ...
    public void setUpPrincipal() {
        ...
        when(identityProviderProvisioning.retrieveByOrigin(ArgumentMatchers.eq(OriginKeys.UAA), ArgumentMatchers.anyString())).thenReturn(uaaProvider);
        when(identityProviderProvisioning.retrieveByOrigin(ArgumentMatchers.eq(OriginKeys.LDAP), ArgumentMatchers.anyString())).thenReturn(new IdentityProvider());
        idps = getIdps();
    ...
    public void check_links_urls(IdentityZone zone) throws Exception {
        ...
        when(mockIDPConfigurator.getIdentityProviderDefinitions((List<String>) ArgumentMatchers.isNull(), ArgumentMatchers.eq(zone))).thenReturn(idps);
    ...
    public void saml_links_for_json() throws Exception {
        LoginInfoEndpoint endpoint = getEndpoint();
        endpoint.setIdpDefinitions(mockIDPConfigurator);
        when(mockIDPConfigurator.getIdentityProviderDefinitions(ArgumentMatchers.anyObject(), ArgumentMatchers.anyObject())).thenReturn(idps);
        endpoint.setIdpDefinitions(mockIDPConfigurator);
        endpoint.infoForJson(model, null, new MockHttpServletRequest("GET", endpoint.getBaseUrl()));
        Map<String, Object> links = (Map<String, Object>) model.asMap().get("links");
        assertEquals("http://someurl", links.get("login"));
    ...
    public void no_usernamePasswordBoxes_if_internalAuth_and_ldap_disabled() throws Exception {
        when(mockIDPConfigurator.getIdentityProviderDefinitions(ArgumentMatchers.anyList(), ArgumentMatchers.anyObject())).thenReturn(idps);
    ...
    public void test_PromptLogic() throws Exception {
        ...
        when(mockIDPConfigurator.getIdentityProviderDefinitions((List<String>) ArgumentMatchers.isNull(), ArgumentMatchers.eq(IdentityZone.getUaa()))).thenReturn(idps);
        endpoint.setIdpDefinitions(mockIDPConfigurator);
        endpoint.infoForJson(model, null, new MockHttpServletRequest("GET", endpoint.getBaseUrl()));
        assertNotNull("prompts attribute should be present", model.get("prompts"));
        assertTrue("prompts should be a Map for JSON content", model.get("prompts") instanceof Map);
    ...
    public void testFilterIDPsForAuthcodeClientInDefaultZone() throws Exception {
        ...
        when(mockIDPConfigurator.getIdentityProviderDefinitions(ArgumentMatchers.eq(allowedProviders), ArgumentMatchers.eq(IdentityZone.getUaa()))).thenReturn(clientIDPs);
    ...
    public void testFilterIDPsForAuthcodeClientInOtherZone() throws Exception {
        ...
        when(mockIDPConfigurator.getIdentityProviderDefinitions(ArgumentMatchers.eq(allowedProviders), ArgumentMatchers.eq(zone))).thenReturn(clientIDPs);
    ...
    public void testFilterIDPsForAuthcodeClientWithNoAllowedIDPsInOtherZone() throws Exception {
        ...
        BaseClientDetails clientDetails = new BaseClientDetails();
        ClientServicesExtension clientDetailsService = mock(ClientServicesExtension.class);
        when(clientDetailsService.loadClientByClientId(ArgumentMatchers.eq("client-id"), ArgumentMatchers.anyString())).thenReturn(clientDetails);
    ...
    public void allowedIdpsforClientOIDCProvider() throws Exception {
        ...
        when(identityProviderProvisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString())).thenReturn(clientAllowedIdps);
    ...
    public void oauth_provider_links_shown() throws Exception {
        ...
        when(identityProviderProvisioning.retrieveAll(ArgumentMatchers.anyBoolean(), ArgumentMatchers.anyString())).thenReturn(Collections.singletonList(identityProvider));
        LoginInfoEndpoint endpoint = getEndpoint();
        endpoint.loginForHtml(model, null, new MockHttpServletRequest(), Collections.singletonList(MediaType.TEXT_HTML));
    ...
    public void we_return_both_oauth_and_oidc_providers() throws Exception {
        ...
        when(identityProviderProvisioning.retrieveAll(ArgumentMatchers.anyBoolean(), ArgumentMatchers.anyString())).thenReturn(Arrays.asList(oauthProvider, oidcProvider));
        LoginInfoEndpoint endpoint = getEndpoint();
    ...
    private LoginInfoEndpoint getEndpoint() {
        ...
        when(emptyConfigurator.getIdentityProviderDefinitionsForZone(ArgumentMatchers.anyObject())).thenReturn(Collections.EMPTY_LIST);
