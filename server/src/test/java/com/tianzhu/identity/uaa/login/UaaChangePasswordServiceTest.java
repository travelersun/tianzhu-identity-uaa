...
import com.tianzhu.identity.uaa.account.UaaChangePasswordService;
...
public class UaaChangePasswordServiceTest {
    ...
    public void testChangePasswordWithUserNotFound() {
        ...
        when(scimUserProvisioning.query(ArgumentMatchers.anyString(), eq(zoneId))).thenReturn(results);
        subject.changePassword("username", "currentPassword", "validPassword");
        verify(passwordValidator).validate("validPassword");
        verify(scimUserProvisioning).query(ArgumentMatchers.anyString(), zoneId);
    }
    ...
    public void changePassword_ReturnsUnprocessableEntity_PasswordNoveltyViolation() {
        ...
        when(scimUserProvisioning.query(ArgumentMatchers.anyString(), eq(IdentityZoneHolder.get().getId()))).thenReturn(results);
    ...
    public void testChangePassword() {
        ...
        subject.changePassword("username", "currentPassword", "validPassword");
        verify(passwordValidator).validate("validPassword");
        verify(scimUserProvisioning).query(ArgumentMatchers.anyString(), eq(IdentityZoneHolder.get().getId()));
