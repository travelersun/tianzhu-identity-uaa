...
import com.tianzhu.identity.uaa.oauth.approval.InMemoryApprovalStore;
...
public class CheckTokenEndpointTests {
    ...
    public void setUp(boolean opaque) throws URISyntaxException {
        ...
        if (opaque) {
            ...
            when(tokenProvisioning.create(any(), ArgumentMatchers.anyString())).thenAnswer(invocation -> {
                RevocableToken token = (RevocableToken) invocation.getArguments()[0];
                tokenMap.put(token.getTokenId(), token);
                return token;
            });
            when(tokenProvisioning.retrieve(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenAnswer(invocation -> {
    ...
    private void mockUserDatabase(String userId, UaaUser user) {
        ...
        when(userDatabase.retrieveUserById(ArgumentMatchers.eq(userId))).thenReturn(user);
        when(userDatabase.retrieveUserById(AdditionalMatchers.not(ArgumentMatchers.eq(userId)))).thenThrow(new UsernameNotFoundException("mock"));
    ...
    public void revokedToken_ThrowsTokenRevokedException() throws Exception {
        setUp();
        when(tokenProvisioning.retrieve(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenThrow(new EmptyResultDataAccessException(1));
