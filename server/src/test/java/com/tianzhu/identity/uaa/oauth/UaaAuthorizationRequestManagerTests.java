...
public class UaaAuthorizationRequestManagerTests {
    ...
    public void testClientIDPAuthorizationInUAAzoneListSucceeds() {
        when(providerProvisioning.retrieveByOrigin(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(MultitenancyFixture.identityProvider("random", "random"));
    ...
    public void testClientIDPAuthorizationInUAAzoneNullProvider() {
        when(providerProvisioning.retrieveByOrigin(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(null);
        client.addAdditionalInformation(ClientConstants.ALLOWED_PROVIDERS, Arrays.asList("random2"));
        factory.checkClientIdpAuthorization(client, user);
    }
    ...
    public void testClientIDPAuthorizationInUAAzoneEmptyResultSetException() {
        when(providerProvisioning.retrieveByOrigin(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenThrow(new EmptyResultDataAccessException(1));
        client.addAdditionalInformation(ClientConstants.ALLOWED_PROVIDERS, Arrays.asList("random2"));
        factory.checkClientIdpAuthorization(client, user);
    }
