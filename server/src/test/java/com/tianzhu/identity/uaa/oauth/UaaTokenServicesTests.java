...
import com.tianzhu.identity.uaa.approval.Approval.ApprovalStatus;
...
import com.tianzhu.identity.uaa.oauth.token.matchers.AbstractOAuth2AccessTokenMatchers;
import com.tianzhu.identity.uaa.oauth.token.matchers.OAuth2RefreshTokenMatchers;
...
import static com.tianzhu.identity.uaa.oauth.TokenTestSupport.*;
import static com.tianzhu.identity.uaa.oauth.UaaTokenServices.UAA_REFRESH_TOKEN;
import static com.tianzhu.identity.uaa.oauth.client.ClientConstants.REQUIRED_USER_GROUPS;
import static com.tianzhu.identity.uaa.oauth.client.ClientDetailsModification.SECRET;
...
import static com.tianzhu.identity.uaa.oauth.token.matchers.OAuth2AccessTokenMatchers.*;
import static com.tianzhu.identity.uaa.user.UaaAuthority.USER_AUTHORITIES;
...
public class UaaTokenServicesTests {
    ...
    public void test_opaque_tokens_are_persisted() throws Exception {
        ...
        verify(tokenProvisioning, times(2)).create(rt.capture(), ArgumentMatchers.anyString());
    ...
    public void test_refresh_token_not_unique_when_set_to_false() {
        ...
        verify(tokenProvisioning, times(0)).deleteRefreshTokensForClientAndUserId(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), eq(currentZoneId));
        verify(tokenProvisioning, times(2)).create(rt.capture(), ArgumentMatchers.anyString());
        RevocableToken refreshToken = rt.getAllValues().get(1);
        assertEquals(RevocableToken.TokenType.REFRESH_TOKEN, refreshToken.getResponseType());
    }
    ...
    public void test_jwt_no_token_is_not_persisted() throws Exception {
        ...
        ArgumentCaptor<RevocableToken> rt = ArgumentCaptor.forClass(RevocableToken.class);
        verify(tokenProvisioning, never()).create(rt.capture(), ArgumentMatchers.anyString());
    ...
    public void test_opaque_refresh_token_is_persisted() throws Exception {
        ...
        ArgumentCaptor<RevocableToken> rt = ArgumentCaptor.forClass(RevocableToken.class);
        verify(tokenProvisioning, times(1)).create(rt.capture(), ArgumentMatchers.anyString());
        assertNotNull(rt.getAllValues());
