...
import static com.tianzhu.identity.uaa.oauth.token.RevocableToken.TokenType.ACCESS_TOKEN;
import static com.tianzhu.identity.uaa.oauth.token.RevocableToken.TokenType.REFRESH_TOKEN;
...
public class JdbcRevocableTokenProvisioningTest extends JdbcTestBase {
    ...
    public void on_application_event_calls_internal_delete_method() throws Exception {
        ...
        for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherZone)) {
            ...
            verify(dao, times(2)).deleteByClient(ArgumentMatchers.eq("id"), ArgumentMatchers.eq(zone.getId()));
        }
    }
