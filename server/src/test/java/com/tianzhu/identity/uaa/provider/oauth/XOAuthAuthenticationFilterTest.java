...
public class XOAuthAuthenticationFilterTest {
    ...
    public void getIdTokenInResponse() throws Exception {
        ...
        Mockito.when(xOAuthAuthenticationManager.authenticate(ArgumentMatchers.anyObject())).thenReturn(authentication);
    ...
    public void redirectsToErrorPageInCaseOfException() throws Exception {
        ...
        Mockito.doThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST, "error from oauth server")).when(xOAuthAuthenticationManager).authenticate(ArgumentMatchers.anyObject());
        filter.doFilter(request, response, chain);
