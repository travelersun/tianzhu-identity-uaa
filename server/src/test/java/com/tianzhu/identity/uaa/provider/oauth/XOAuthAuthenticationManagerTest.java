...
package com.tianzhu.identity.uaa.provider.oauth;

import com.fasterxml.jackson.core.type.TypeReference;
import org.apache.commons.codec.binary.Base64;
import com.tianzhu.identity.uaa.authentication.AccountNotPreCreatedException;
import com.tianzhu.identity.uaa.authentication.UaaAuthentication;
import com.tianzhu.identity.uaa.authentication.manager.ExternalGroupAuthorizationEvent;
import com.tianzhu.identity.uaa.authentication.manager.InvitedUserAuthenticatedEvent;
import com.tianzhu.identity.uaa.authentication.manager.NewUserAuthenticatedEvent;
...
import com.tianzhu.identity.uaa.oauth.jwk.JsonWebKey;
import com.tianzhu.identity.uaa.oauth.jwk.JsonWebKeySet;
...
import com.tianzhu.identity.uaa.oauth.token.VerificationKeyResponse;
import com.tianzhu.identity.uaa.oauth.token.VerificationKeysListResponse;
...
import com.tianzhu.identity.uaa.user.*;
...
import static com.tianzhu.identity.uaa.impl.config.LegacyTokenKey.LEGACY_TOKEN_KEY_ID;
import static com.tianzhu.identity.uaa.provider.ExternalIdentityProviderDefinition.GROUP_ATTRIBUTE_NAME;
...
import static com.tianzhu.identity.uaa.util.UaaMapUtils.map;
...
public class XOAuthAuthenticationManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        for(String scope : SCOPES_LIST) {
            ...
            when(externalMembershipManager.getExternalGroupMapsByExternalGroup(ArgumentMatchers.eq(scope), ArgumentMatchers.anyString(), ArgumentMatchers.anyString()))
        ...
        when(restTemplateFactory.getRestTemplate(ArgumentMatchers.anyBoolean())).thenReturn(new RestTemplate());
    ...
    public void when_origin_provided_no_call_resolve_based_on_issuer() {
        ...
        verify(xoAuthAuthenticationManager, never()).resolveOriginProvider(ArgumentMatchers.anyString(), ArgumentMatchers.anyString());
    }
    ...
    public void unable_to_resolve_to_single_provider() throws Exception {
        ...
        when(provisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.eq(zoneId))).thenReturn(emptyList());
        getAuthenticationData(xCodeToken);
    }
    ...
    public void origin_is_resolved_based_on_issuer_and_id_token() throws Exception {
        CompositeAccessToken token = getCompositeAccessToken();
        xCodeToken = new XOAuthCodeToken(null,null,null,token.getIdTokenValue(),null,null);
        when(provisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString())).thenReturn(Arrays.asList(getProvider()));
        //perform test
        getAuthenticationData(xCodeToken);

        ArgumentCaptor<String> idTokenCaptor = ArgumentCaptor.forClass(String.class);
        verify(xoAuthAuthenticationManager, times(1)).resolveOriginProvider(idTokenCaptor.capture(), ArgumentMatchers.anyObject());
        verify(provisioning, never()).retrieveByOrigin(ArgumentMatchers.anyString(), ArgumentMatchers.anyString());
        verify(xoAuthProviderConfigurator, times(1)).retrieveByIssuer(ArgumentMatchers.eq("http://oidc10.uaa-acceptance.cf-app.com/oauth/token"), ArgumentMatchers.anyString());
    ...
    public void if_internal_idp_use_local_keys() throws Exception {
        ...
        verify(xoAuthAuthenticationManager, times(1)).getClaimsFromToken(ArgumentMatchers.same(xCodeToken), ArgumentMatchers.anyObject());
        verify(xoAuthAuthenticationManager, times(1)).getClaimsFromToken(ArgumentMatchers.eq(idToken), ArgumentMatchers.anyObject());
    ...
    public void discoveryURL_is_used() throws MalformedURLException {
        ...
        when(provisioning.retrieveByOrigin(ArgumentMatchers.eq(ORIGIN), ArgumentMatchers.anyString())).thenReturn(identityProvider);

        mockToken();
        addTheUserOnAuth();
        xoAuthAuthenticationManager.authenticate(xCodeToken);
        verify(xoAuthProviderConfigurator, atLeast(1)).overlay(ArgumentMatchers.eq(config));
    ...
    public void idToken_In_Redirect_Should_Use_it() throws Exception {
        ...
        verify(xoAuthAuthenticationManager, times(1)).getClaimsFromToken(ArgumentMatchers.same(xCodeToken), ArgumentMatchers.anyObject());
        verify(xoAuthAuthenticationManager, times(1)).getClaimsFromToken(ArgumentMatchers.eq(idToken), ArgumentMatchers.anyObject());
        verify(xoAuthAuthenticationManager, never()).getRestTemplate(ArgumentMatchers.anyObject());
    ...
    public void failsIfProviderIsNotOIDCOrOAuth() throws Exception {
        when(provisioning.retrieveByOrigin(ArgumentMatchers.eq(ORIGIN), ArgumentMatchers.anyString())).thenReturn(MultitenancyFixture.identityProvider("the_origin", "uaa"));
    ...
    public void failsIfProviderIsNotFound() throws Exception {
        when(provisioning.retrieveByOrigin(ArgumentMatchers.eq(ORIGIN), ArgumentMatchers.anyString())).thenReturn(null);
        Authentication authentication = xoAuthAuthenticationManager.authenticate(xCodeToken);
        assertNull(authentication);
    }
