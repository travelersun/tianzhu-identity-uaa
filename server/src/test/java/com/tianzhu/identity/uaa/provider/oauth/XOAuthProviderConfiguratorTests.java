...
public class XOAuthProviderConfiguratorTests {
    ...
    public void setup() throws MalformedURLException {
        ...
        when(cache.getUrlContent(ArgumentMatchers.anyString(), ArgumentMatchers.anyObject())).thenReturn(jsonResponse.getBytes());
        ...
        when(provisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString())).thenReturn(Arrays.asList(oidcProvider, oauthProvider, new IdentityProvider<>().setType(LDAP)));
    }
    ...
    public void retrieveAll() {
        ...
        verify(configurator, times(1)).overlay(ArgumentMatchers.eq(config));
    }
    ...
    public void retrieveActive() {
        ...
        assertEquals(2, activeXOAuthProviders.size());
        verify(configurator, times(1)).overlay(ArgumentMatchers.eq(config));
        verify(configurator, times(1)).retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString());
    }
    ...
    public void issuer_not_found() throws Exception {
        ...
        reset(provisioning);
        when(provisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString())).thenReturn(Arrays.asList(oauthProvider, new IdentityProvider<>().setType(LDAP)));
        IdentityProvider<OIDCIdentityProviderDefinition> activeXOAuthProvider = configurator.retrieveByIssuer(issuer, IdentityZone.getUaa().getId());
        assertEquals(issuer, activeXOAuthProvider.getConfig().getIssuer());
        verify(configurator, times(0)).overlay(ArgumentMatchers.eq(config));
        verify(configurator, times(1)).retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString());
    }
    ...
    public void duplicate_issuer_found() throws Exception {
        ...
        reset(provisioning);
        when(provisioning.retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString())).thenReturn(Arrays.asList(oidcProvider, oidcProvider, oauthProvider, new IdentityProvider<>().setType(LDAP)));
        IdentityProvider<OIDCIdentityProviderDefinition> activeXOAuthProvider = configurator.retrieveByIssuer(issuer, IdentityZone.getUaa().getId());
        assertEquals(issuer, activeXOAuthProvider.getConfig().getIssuer());
        verify(configurator, times(2)).overlay(ArgumentMatchers.eq(config));
        verify(configurator, times(1)).retrieveAll(ArgumentMatchers.eq(true), ArgumentMatchers.anyString());
    }

    @Test
    public void retrieveByOrigin() {
        when(provisioning.retrieveByOrigin(ArgumentMatchers.eq(OIDC10), ArgumentMatchers.anyString())).thenReturn(oidcProvider);
        when(provisioning.retrieveByOrigin(ArgumentMatchers.eq(OAUTH20), ArgumentMatchers.anyString())).thenReturn(oauthProvider);
        ...
        verify(configurator, never()).overlay(ArgumentMatchers.anyObject());
    }

    @Test
    public void retrieveById() {
        when(provisioning.retrieve(ArgumentMatchers.eq(OIDC10), ArgumentMatchers.anyString())).thenReturn(oidcProvider);
        when(provisioning.retrieve(ArgumentMatchers.eq(OAUTH20), ArgumentMatchers.anyString())).thenReturn(oauthProvider);
    ...
    public void overlay_withOverrideValues() throws MalformedURLException {
        ...
        verify(cache).getUrlContent(ArgumentMatchers.eq(discoveryUrl), ArgumentMatchers.any());
        verify(factory).getRestTemplate(ArgumentMatchers.eq(true));
    }
    ...
    public void overlay_withoutOverrideValues() throws MalformedURLException {
        ...
        verify(cache).getUrlContent(ArgumentMatchers.any(), ArgumentMatchers.any());
        verify(factory).getRestTemplate(ArgumentMatchers.eq(false));
    }
    ...
    public void excludeUnreachableOidcProvider() {
        when(cache.getUrlContent(ArgumentMatchers.anyString(), ArgumentMatchers.anyObject())).thenReturn(null);
