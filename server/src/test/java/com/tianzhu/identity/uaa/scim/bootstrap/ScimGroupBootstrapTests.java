...
public class ScimGroupBootstrapTests extends JdbcTestBase {
    ...
    public void allowsBootstrapFromOtherInstance() throws Exception {
        ...
        when(gDB.create(any(), ArgumentMatchers.anyString())).thenReturn(multipleBootstrapGroupBefore);
        when(gDB.createOrGet(any(), ArgumentMatchers.anyString())).thenReturn(multipleBootstrapGroupBefore);
        when(gDB.update(ArgumentMatchers.anyString(), any(), ArgumentMatchers.anyString())).thenThrow(new IncorrectResultSizeDataAccessException(1, 0));
        when(gDB.getByName(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(multipleBootstrapGroupAfter);
