...
import com.tianzhu.identity.uaa.approval.JdbcApprovalStore;
...
public class ScimUserEndpointsTests {
    ...
    public void setUp() {
        ...
        doThrow(new InvalidPasswordException("Password must be at least 1 characters in length."))
            .when(mockPasswordValidator).validate(ArgumentMatchers.eq(""));
    ...
    public void createUser_whenPasswordIsInvalid_throwsException() {
        doThrow(new InvalidPasswordException("whaddup")).when(mockPasswordValidator).validate(ArgumentMatchers.anyString());
        ScimUserProvisioning mockDao = mock(ScimUserProvisioning.class);
        endpoints.setScimUserProvisioning(mockDao);
        String zoneId = IdentityZoneHolder.get().getId();
        when(mockDao.createUser(ArgumentMatchers.any(ScimUser.class), ArgumentMatchers.anyString(), ArgumentMatchers.eq(zoneId))).thenReturn(new ScimUser());
    ...
    public void testFindUsersGroupsSyncedByDefault() throws Exception {
        ...
        verify(mockgroupMembershipManager, atLeastOnce()).getGroupsWithMember(ArgumentMatchers.anyString(), ArgumentMatchers.anyBoolean(), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()));
    ...
    public void testFindUsersApprovalsSyncedByDefault() throws Exception {
        ...
        endpoints.findUsers("", "id pr", null, "ascending", 1, 100);
        verify(mockApprovalStore, atLeastOnce()).getApprovalsForUser(ArgumentMatchers.anyString(), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()));
    ...
    public void testCreateUserWithEmailDomainNotAllowedForOriginUaa() {
        ...
        when(identityProviderProvisioning.retrieveActive(ArgumentMatchers.anyString())).thenReturn(asList(ldapProvider, oidcProvider));

        expected.expect(ScimException.class);
        expected.expectMessage("The user account is set up for single sign-on. Please use one of these origin(s) : [ldap, oidc1]");
        endpoints.createUser(user, new MockHttpServletRequest(), new MockHttpServletResponse());
        verify(identityProviderProvisioning).retrieveActive(ArgumentMatchers.anyString());
    }
    ...
    public void testCreateUserWithEmailDomainAllowedForOriginNotUaa() {
        ...
        IdentityProvider ldapProvider = new IdentityProvider().setActive(true).setType(OriginKeys.LDAP).setOriginKey(OriginKeys.LDAP).setConfig(new LdapIdentityProviderDefinition());
        ldapProvider.getConfig().setEmailDomain(Collections.singletonList("example.org"));
        when(identityProviderProvisioning.retrieveActive(ArgumentMatchers.anyString())).thenReturn(asList(ldapProvider));

        endpoints.createUser(user, new MockHttpServletRequest(), new MockHttpServletResponse());
        verify(identityProviderProvisioning, times(0)).retrieveActive(ArgumentMatchers.anyString());
    }
    ...
    public void testWhenEmailDomainConfiguredForUaaAllowsCreationOfUser() {
        ...
        when(identityProviderProvisioning.retrieveActive(ArgumentMatchers.anyString())).thenReturn(asList(uaaProvider));
