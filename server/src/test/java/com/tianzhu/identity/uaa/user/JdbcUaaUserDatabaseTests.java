...
import com.tianzhu.identity.uaa.test.TestUtils;
...
import static com.tianzhu.identity.uaa.user.JdbcUaaUserDatabase.*;
...
public class JdbcUaaUserDatabaseTests extends JdbcTestBase {
    ...
    public void is_the_right_query_used() throws Exception {
        ...
        verify(template).queryForObject(ArgumentMatchers.eq(DEFAULT_CASE_SENSITIVE_USER_BY_USERNAME_QUERY), ArgumentMatchers.eq(db.getMapper()), ArgumentMatchers.eq(username.toLowerCase()), ArgumentMatchers.eq(true), ArgumentMatchers.eq(OriginKeys.UAA), ArgumentMatchers.eq(OriginKeys.UAA));
        db.retrieveUserByEmail(username, OriginKeys.UAA);
        verify(template).query(ArgumentMatchers.eq(DEFAULT_CASE_SENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY), ArgumentMatchers.eq(db.getMapper()), ArgumentMatchers.eq(username.toLowerCase()), ArgumentMatchers.eq(true), ArgumentMatchers.eq(OriginKeys.UAA), ArgumentMatchers.eq(OriginKeys.UAA));

        db.setCaseInsensitive(true);

        db.retrieveUserByName(username, OriginKeys.UAA);
        verify(template).queryForObject(ArgumentMatchers.eq(DEFAULT_CASE_INSENSITIVE_USER_BY_USERNAME_QUERY), ArgumentMatchers.eq(db.getMapper()), ArgumentMatchers.eq(username.toLowerCase()), ArgumentMatchers.eq(true), ArgumentMatchers.eq(OriginKeys.UAA), ArgumentMatchers.eq(OriginKeys.UAA));
        db.retrieveUserByEmail(username, OriginKeys.UAA);
        verify(template).query(ArgumentMatchers.eq(DEFAULT_CASE_INSENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY), ArgumentMatchers.eq(db.getMapper()), ArgumentMatchers.eq(username.toLowerCase()), ArgumentMatchers.eq(true), ArgumentMatchers.eq(OriginKeys.UAA), ArgumentMatchers.eq(OriginKeys.UAA));
    }
    ...
    public void getUserWithMultipleExtraAuthorities() {
        ...
        UaaUser joe = db.retrieveUserByName("joe", OriginKeys.UAA);
        verify(spy, times(2)).queryForList(ArgumentMatchers.anyString(), Matchers.<String>anyVararg());
        assertTrue("authorities does not contain uaa.user",
                joe.getAuthorities().contains(new SimpleGrantedAuthority("uaa.user")));
