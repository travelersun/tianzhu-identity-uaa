...
import com.tianzhu.identity.uaa.authentication.UaaAuthenticationTestFactory;
...
import static com.tianzhu.identity.uaa.zone.IdentityZoneHolder.isUaa;
import static com.tianzhu.identity.uaa.zone.MultitenantJdbcClientDetailsService.DEFAULT_DELETE_STATEMENT;
...
public class MultitenantJdbcClientDetailsServiceTests extends JdbcTestBase {
    ...
    public void event_calls_delete_method() throws Exception {
        ...
        verify(service, times(1)).deleteByClient(ArgumentMatchers.eq(client.getClientId()), ArgumentMatchers.eq(IdentityZoneHolder.get().getId()));
    }
    ...
    public void delete_by_client_id() throws Exception {
        ...
        for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherIdentityZone)) {
            ...
            verify(service, times(1)).deleteByClient(ArgumentMatchers.eq("some-client-id"), ArgumentMatchers.eq(zone.getId()));
            reset(service);
        }
    }
    ...
    public void delete_by_client_respects_zone_id_param() throws Exception {
        ...
        for (IdentityZone zone : Arrays.asList(IdentityZone.getUaa(), otherIdentityZone)) {
            reset(service);
            reset(template);
            doReturn(1).when(template).update(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString());
            IdentityZoneHolder.set(zone);
            try {
                service.deleteByClient("some-client-id", "zone-id");
            } catch (Exception e) {
            }
            verify(service, times(1)).deleteByClient(ArgumentMatchers.eq("some-client-id"), ArgumentMatchers.eq("zone-id"));
