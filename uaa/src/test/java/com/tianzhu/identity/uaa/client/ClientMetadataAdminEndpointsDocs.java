...
import com.tianzhu.identity.uaa.mock.clients.AdminClientCreator;
...
public class ClientMetadataAdminEndpointsDocs extends AdminClientCreator {
  ...
  private Snippet responseFields = PayloadDocumentation.responseFields(
    SnippetUtils.fieldWithPath("clientId").description(CLIENT_ID_DESC),
    SnippetUtils.fieldWithPath("showOnHomePage").description(SHOW_ON_HOME_PAGE_DESC),
    SnippetUtils.fieldWithPath("appLaunchUrl").description(APP_LAUNCH_URL_DESC),
    SnippetUtils.fieldWithPath("appIcon").description(APP_ICON_DESC),
    SnippetUtils.fieldWithPath("createdBy").description(RESOURCE_OWNER_GUID).type(JsonFieldType.STRING).optional()
  );
  ...
  public void getAllClientMetadata() throws Exception {
    ...
    Snippet responseFields = PayloadDocumentation.responseFields(
      SnippetUtils.fieldWithPath("[].clientId").description(CLIENT_ID_DESC),
      SnippetUtils.fieldWithPath("[].clientName").description(CLIENT_NAME_DESC),
      SnippetUtils.fieldWithPath("[].showOnHomePage").description(SHOW_ON_HOME_PAGE_DESC),
      SnippetUtils.fieldWithPath("[].appLaunchUrl").description(APP_LAUNCH_URL_DESC),
      SnippetUtils.fieldWithPath("[].appIcon").description(APP_ICON_DESC),
      SnippetUtils.fieldWithPath("[].createdBy").description(RESOURCE_OWNER_GUID)
    );
