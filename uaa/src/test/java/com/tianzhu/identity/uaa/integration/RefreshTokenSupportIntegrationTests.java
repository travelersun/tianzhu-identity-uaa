...
import com.tianzhu.identity.uaa.test.TestAccountSetup;
...
public class RefreshTokenSupportIntegrationTests {
    ...
    public void testTokenRefreshedCorrectFlow() throws Exception {
        ...
        ResponseEntity<Void> result = serverRunning.getForResponse(uri.toString(), IntegrationTestUtils.getHeaders(cookies));
        ...
        ResponseEntity<String> response = serverRunning.getForString(location, IntegrationTestUtils.getHeaders(cookies));
        ...
        result = serverRunning.postForResponse("/login.do", IntegrationTestUtils.getHeaders(cookies), formData);
        cookies.clear();
        if (result.getHeaders().containsKey("Set-Cookie")) {
            for (String cookie : result.getHeaders().get("Set-Cookie")) {
                int nameLength = cookie.indexOf('=');
        ...
        response = serverRunning.getForString(result.getHeaders().getLocation().toString(), IntegrationTestUtils.getHeaders(cookies));
        if (response.getHeaders().containsKey("Set-Cookie")) {
            for (String cookie : response.getHeaders().get("Set-Cookie")) {
                int nameLength = cookie.indexOf('=');
                cookies.addCookie(new BasicClientCookie(cookie.substring(0, nameLength), cookie.substring(nameLength+1)));
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            ...
            formData.add(DEFAULT_CSRF_COOKIE_NAME, IntegrationTestUtils.extractCookieCsrf(response.getBody()));
            result = serverRunning.postForResponse("/oauth/authorize", IntegrationTestUtils.getHeaders(cookies), formData);
            assertEquals(HttpStatus.FOUND, result.getStatusCode());
