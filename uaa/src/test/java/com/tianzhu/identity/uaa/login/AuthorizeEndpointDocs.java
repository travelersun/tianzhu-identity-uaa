...
public class AuthorizeEndpointDocs extends InjectedMockContextTest {
    private final ParameterDescriptor clientIdParameter = SnippetUtils.parameterWithName(CLIENT_ID).description("a unique string representing the registration information provided by the client").attributes(key("constraints").value("Required"), key("type").value(STRING));
    private final ParameterDescriptor scopesParameter = SnippetUtils.parameterWithName(SCOPE).description("requested scopes, space-delimited").attributes(key("constraints").value("Optional"), key("type").value(STRING));
    private final ParameterDescriptor redirectParameter = SnippetUtils.parameterWithName(REDIRECT_URI).description("redirection URI to which the authorization server will send the user-agent back once access is granted (or denied), optional if pre-registered by the client").attributes(key("constraints").value("Optional"), key("type").value(STRING));
    private final ParameterDescriptor promptParameter = SnippetUtils.parameterWithName(ID_TOKEN_HINT_PROMPT).description("specifies whether to prompt for user authentication. Only value `"+ID_TOKEN_HINT_PROMPT_NONE+"` is supported.").attributes(key("constraints").value("Optional"), key("type").value(STRING));
    private final ParameterDescriptor responseTypeParameter = SnippetUtils.parameterWithName(RESPONSE_TYPE).attributes(key("constraints").value("Required"), key("type").value(STRING));
    ...
    public void apiCodeRequest() throws Exception {
        ...
        Snippet requestParameters = RequestDocumentation.requestParameters(
            ...
            clientIdParameter,
            redirectParameter,
            SnippetUtils.parameterWithName(STATE).description("any random string to be returned in the Location header as a query parameter, used to achieve per-request customization").attributes(key("constraints").value("Required"), key("type").value(STRING))
        );
