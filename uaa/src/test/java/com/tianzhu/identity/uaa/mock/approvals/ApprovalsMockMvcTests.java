...
package com.tianzhu.identity.uaa.mock.approvals;
...
public class ApprovalsMockMvcTests extends AbstractTokenMockMvcTests {
    ...
    public void test_post_approval_csrf() throws Exception {
        ...
        getMockMvc().perform(
            post.with(MockMvcUtils.CookieCsrfPostProcessor.cookieCsrf().useInvalidToken())
        ).andExpect(status().isForbidden());

        getMockMvc().perform(
            post.with(MockMvcUtils.CookieCsrfPostProcessor.cookieCsrf())
        )
            .andExpect(status().isFound())
