...
import com.tianzhu.identity.uaa.mock.DefaultConfigurationTestSuite;
import com.tianzhu.identity.uaa.mock.util.ApacheDSHelper;
import com.tianzhu.identity.uaa.provider.IdentityProvider;
import com.tianzhu.identity.uaa.provider.IdentityProviderValidationRequest;
import com.tianzhu.identity.uaa.provider.IdentityProviderValidationRequest.UsernamePasswordAuthentication;
...
import com.tianzhu.identity.uaa.util.UaaStringUtils;
...
import static com.tianzhu.identity.uaa.provider.ldap.ProcessLdapProperties.NONE;
import static com.tianzhu.identity.uaa.provider.ldap.ProcessLdapProperties.SIMPLE;
...
public class LdapMockMvcTests  {
    ...
    public void createTestZone() throws Exception {
        ...
        zone = MockMvcUtils.utils().createZoneForInvites(getMockMvc(), getWebApplicationContext(), clientId, REDIRECT_URI);
    ...
    public void acceptInvitation_for_ldap_user_whose_username_is_not_email() throws Exception {
        ...
        URL url = MockMvcUtils.utils().inviteUser(
        ...
        String code = MockMvcUtils.utils().extractInvitationCode(url.toString());
        ...
        url = MockMvcUtils.utils().inviteUser(getWebApplicationContext(), getMockMvc(), email, zone.getAdminToken(), zone.getZone().getIdentityZone().getSubdomain(), zone.getScimInviteClient().getClientId(), LDAP, REDIRECT_URI);
        code = MockMvcUtils.utils().extractInvitationCode(url.toString());
    ...
    public void testLdapConfigurationBeforeSave() throws Exception {
        ...
        String identityAccessToken = MockMvcUtils.utils().getClientOAuthAccessToken(getMockMvc(), "identity", "identitysecret", "");
        String adminAccessToken = MockMvcUtils.utils().getClientOAuthAccessToken(getMockMvc(), "admin", "adminsecret", "");
        IdentityZone zone = MockMvcUtils.utils().createZoneUsingWebRequest(getMockMvc(), identityAccessToken);
        String zoneAdminToken = MockMvcUtils.utils().getZoneAdminToken(getMockMvc(), adminAccessToken, zone.getId());
    ...
    public void testLoginInNonDefaultZone() throws Exception {
        ...
        MockMvcUtils.utils().createIdpUsingWebRequest(getMockMvc(), zone.getZone().getIdentityZone().getId(), zone.getZone().getZoneAdminToken(), provider, status().isOk(), true);
    ...
    public void validateOriginForNonLdapUser() throws Exception {
        ...
        MockMvcUtils.utils().createUserInZone(getMockMvc(), zone.getAdminToken(), user, zone.getZone().getIdentityZone().getSubdomain());
    ...
    public void testStopIfException() throws Exception {
        if (ldapProfile.equals("ldap-simple-bind.xml") && ldapGroup.equals("ldap-groups-null.xml")) {
            ...
            user = MockMvcUtils.utils().createUserInZone(getMockMvc(), zone.getAdminToken(), user, zone.getZone().getIdentityZone().getSubdomain());
    ...
    public void updateLdapProvider() throws Exception {
        provider = MockMvcUtils.utils().createIdpUsingWebRequest(
            getMockMvc(),
