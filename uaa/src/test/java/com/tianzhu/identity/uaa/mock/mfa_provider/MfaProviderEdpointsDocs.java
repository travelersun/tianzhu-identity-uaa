...
import com.tianzhu.identity.uaa.mfa_provider.JdbcMfaProviderProvisioning;
...
import static com.tianzhu.identity.uaa.mfa_provider.MfaProvider.MfaProviderType.GOOGLE_AUTHENTICATOR;
...
public class MfaProviderEdpointsDocs extends InjectedMockContextTest {
    ...
    private static final FieldDescriptor ACTIVE = SnippetUtils.fieldWithPath("active").optional(true).type(BOOLEAN).description(ACTIVE_DESC);
    private static final FieldDescriptor NAME = SnippetUtils.fieldWithPath("name").required().type(STRING).description(NAME_DESC);
    private static final FieldDescriptor CONFIG = SnippetUtils.fieldWithPath("config").required().description("Configuration of MFA providers");
    private static final FieldDescriptor TYPE = SnippetUtils.fieldWithPath("type").required().type(STRING).description("Type of MFA providers");
    private static final FieldDescriptor LAST_MODIFIED = SnippetUtils.fieldWithPath("last_modified").description(LAST_MODIFIED_DESC);
    private static final FieldDescriptor ID = SnippetUtils.fieldWithPath("id").type(STRING).description(ID_DESC);
    private static final FieldDescriptor CREATED = SnippetUtils.fieldWithPath("created").description(CREATED_DESC);
    private static final FieldDescriptor IDENTITY_ZONE_ID = SnippetUtils.fieldWithPath("identityZoneId").type(STRING).description(IDENTITY_ZONE_ID_DESC);
    ...
    public void testCreateGoogleMfaProvider() throws Exception {
        ...
        FieldDescriptor[] idempotentFields = (FieldDescriptor[]) ArrayUtils.addAll(commonProviderFields, new FieldDescriptor[]{
                SnippetUtils.fieldWithPath("config.algorithm").optional("SHA256").type(STRING).description("Algorithm"),
                SnippetUtils.fieldWithPath("config.providerDescription").optional(null).type(STRING).description("provider description"),
                SnippetUtils.fieldWithPath("config.digits").optional(6).type(NUMBER).description("digits"),
                SnippetUtils.fieldWithPath("config.issuer").optional(null).type(STRING).description("issuer. mention default behavior"),
                SnippetUtils.fieldWithPath("config.duration").optional(30).type(NUMBER).description("duration")
        });
