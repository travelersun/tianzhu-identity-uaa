...
public class UserIdConversionEndpointDocs extends InjectedMockContextTest{
    ...
    public void lookUpIds() throws Exception {
        ...
        Snippet requestParams = RequestDocumentation.requestParameters(
            SnippetUtils.parameterWithName("filter").required().description("SCIM filter for users over `userName`, `id`, and `origin`, using only the `eq` comparison operator").attributes(key("type").value(STRING)),
            SnippetUtils.parameterWithName("sortOrder").optional("ascending").description("sort by username in `ascending` or `descending` order").attributes(key("type").value(STRING)),
            SnippetUtils.parameterWithName("startIndex").optional("1").description("display paged results beginning at specified index").attributes(key("type").value(NUMBER)),
            SnippetUtils.parameterWithName("count").optional("100").description("number of results to return per page").attributes(key("type").value(NUMBER)),
            SnippetUtils.parameterWithName("includeInactive").optional("false").description("include users from inactive identity providers").attributes(key("type").value(BOOLEAN))
        );
        ...
        Snippet responseFields = PayloadDocumentation.responseFields(
            ...
            SnippetUtils.fieldWithPath("resources[].id").description("The globally unique identifier for this user"),
            SnippetUtils.fieldWithPath("resources[].userName").description("The username"),
            SnippetUtils.fieldWithPath("resources[].origin").description("The origin of the user, e.g. an identity provider alias")
        );
